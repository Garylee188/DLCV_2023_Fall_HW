...
train_tfm = transforms.Compose([
    ...
    transforms.RandomCrop(128, 50, True),
    transforms.ToTensor(),
...
class MyDataset(Dataset):
    ...
    def __init__(self, data_path, images_list, transform=None):
...
def model_training(train_data_path, valid_data_path, batch_size=4, epochs=50, learning_rate=0.0001):
    train_images = []
    ...
    train_dataset = MyDataset(train_data_path, train_images, train_tfm)
    valid_dataset = MyDataset(valid_data_path, valid_images, valid_tfm)
    ...
    backbone.load_state_dict(torch.load(f'G:/DLCV_HW1/hw1_p2_result/100_byol_ssl_backbone.pt'))
#     backbone.load_state_dict(torch.load(f'G:/DLCV_HW1/hw1_data_4_students/hw1_data/p2_data/pretrain_model_SL.pt'))
...
#         param.requires_grad = False
    ...
    model.to(device)
    modelName = f"Setting_C_new.ckpt"
    
    path = f"{modelName[:-5]}.txt"
    f = open(path, 'w')
    f.write(f"{modelName}\n")
    f.write(f"Epochs={epochs}\n")
    f.write(f"batch_size={batch_size}\n")
    f.write(f"learning_rate={learning_rate}\n")
    f.write(f"image size=128\n")
    f.close()
...
#         scheduler.step()
        ...
        if valid_loss < min_loss:
            ...
            torch.save(model.state_dict(), f"G:/DLCV_HW1/hw1_p2_result/{modelName}")  # Save model to specified path
            notImprove = 0
        else:
        ...
        torch.save(model.state_dict(), f"G:/DLCV_HW1/hw1_p2_result/{epoch+1}_{modelName}")
#         scheduler.step()
...
#             break
    ...
    np.savetxt(f"G:/DLCV_HW1/hw1_p2_result/{modelName[:-5]}_train_log.csv", training_log, delimiter=",")


if __name__ == "__main__":
    train_set = f"G:/DLCV_HW1/hw1_data_4_students/hw1_data/p2_data/office/train"
    val_set = f"G:/DLCV_HW1/hw1_data_4_students/hw1_data/p2_data/office/val"
