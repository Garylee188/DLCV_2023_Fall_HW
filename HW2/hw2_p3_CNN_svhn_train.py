...
def get_data(batch_size=32):
    ...
    usps_train_set = Digit_Dataset(usps_data_path, usps_train_csv, train_tfm)
    usps_valid_set = Digit_Dataset(usps_data_path, usps_valid_csv, valid_tfm)
...
class LabelPredictor(nn.Module):
    def __init__(self, in_channel=64, in_dim=7, labels_num=10):
        ...
        self.fc = nn.Sequential(
            nn.Linear(in_channel*in_dim*in_dim, 512),
            nn.ReLU(),
            nn.Linear(512, 256),
            nn.ReLU(),
...
class DomainClassifier(nn.Module):
    def __init__(self, in_channel=64, in_dim=7):
        ...
        self.fc = nn.Sequential(
            nn.Linear(in_channel*in_dim*in_dim, 128),
            nn.ReLU(),
            nn.Linear(128, 64),
            nn.ReLU(),
            nn.Linear(64, 1),
...
#         domain_scheduler.step()
        ...
        if (epoch+1) % 10 == 0:
            F_model_name = str(epoch+1) + '_Feature_Extractor_Upper.ckpt'  # Upper or Lower
...
if __name__ == '__main__':
    ...
    save_dir = r"E:\DLCV-HW2\p3_result\target_svhn_2"
    ...
    train_eval(
        ...
        label_predictor = L,
        domain_classifier = D,
        src_train_loader = data["svhn_train_loader"],  # Lower: mnistm_train_loader, Upper: svhn_train_loader
        src_valid_loader = data["mnistm_valid_loader"],
        tgt_train_loader = data["svhn_train_loader"],
        tgt_valid_loader = data["svhn_valid_loader"],
        n_epochs = n_epochs,
        lr = lr,
    ...
    F.load_state_dict(torch.load(r'E:\DLCV-HW2\p3_result\target_svhn_2\20_Feature_Extractor_Upper.ckpt'))  # Upper or Lower
    L.load_state_dict(torch.load(r'E:\DLCV-HW2\p3_result\target_svhn_2\20_Label_Predictor_Upper.ckpt'))
    valid_loss, valid_acc = evaluation(
        feature_extractor = F,
        label_predictor = L,
        label_criterion = nn.CrossEntropyLoss(),
        valid_loader = data["svhn_valid_loader"],
        device = device
    )
    print(valid_acc)
